...
# z 계산 함수
def compute_z(x, y, atk, defense, w):
    numerator = atk ** 2
    denominator = atk + defense * (1 - w * 0.01)
    return (numerator / denominator) * (1 + x * 0.01) * 1.2 * (100 * 0.01) * (y * 0.01)

# 슬라이더
atk = st.slider("🗡 Attack", 1, 8000, 1000, step=50)
defense = st.slider("🛡 Enemy Defense", 1, 7000, 1000, step=50)
w = st.slider("💥 Weakening %", 0.0, 100.0, 50.0, step=1.0)
x = st.slider("📊 Damage Increase %", 0.0, 400.0, 100.0, step=5.0)
y = st.slider("🎯 Crit %", 0.0, 400.0, 100.0, step=5.0)
